...
def dice_score(pred, target, smooth=1):
    ...
    dice_coef = (2. * intersection + smooth) / (pred.sum(dim=2).sum(dim=2) + target.sum(dim=2).sum(dim=2) + smooth)
...
# Modify the train function to include the dice_score and iou_score in the saved state
def train(train_loader, model, criterion, optimizer, validation, args):
    ...
    for epoch in range(epoch, args.n_epoch + 1):
        ...
        valid_dice = valid_metrics['dice_score']
        valid_iou = valid_metrics['iou_score']
        
        # Store metrics
        metrics['train_loss'].append(losses.avg)
        metrics['valid_loss'].append(valid_loss)
        metrics['dice_score'].append(valid_dice)
        metrics['iou_score'].append(valid_iou)

        valid_losses.append(valid_loss)
        print(f'\tvalid_loss = {valid_loss:.5f}, dice_score = {valid_dice:.5f}, iou_score = {valid_iou:.5f}')
        tq.close()
        ...
        torch.save({
            ...
            'dice_score': dice_score(masks_pred, target_var).cpu().item(),  # use .cpu() before .item()
            'iou_score': iou_score(masks_pred, target_var).cpu().item()
        }, epoch_model_path)
        ...
        if valid_loss < min_val_los:
            ...
            torch.save({
                ...
                'valid_loss': valid_loss,
                'train_loss': losses.avg,
                'dice_score': dice_score(masks_pred, target_var).cpu().item(),
                'iou_score': iou_score(masks_pred, target_var).cpu().item()
            }, best_model_path)

# Modify the validate function to calculate and return the dice_score and iou_score
def validate(model, val_loader, criterion):
    ...
    with torch.no_grad():
        ...
        for i, (input, target) in enumerate(val_loader):
            ...
            dice_scores.update(dice_score(output, target_var).item(), input_var.size(0))
            iou_scores.update(iou_score(output, target_var).item(), input_var.size(0))

    return {
        'valid_loss': losses.avg, 
        'dice_score': dice_scores.avg, 
        'iou_score': iou_scores.avg
    }
